...
class TestTgtgServiceMonitor:
    ...
    def test_start_monitoring_success(self, monitoring_service):
        ...
        with patch.object(monitoring_service, '_get_tgtg_service_instance', return_value=mock_tgtg_service):
            monitoring_service.start_monitoring()
            mock_tgtg_service.get_favorites_items.assert_called_once()

    def test_start_monitoring_no_credentials(self, monitoring_service):
        with patch.object(monitoring_service, '_get_tgtg_service_instance', return_value=None), patch("app.common.logger.LOGGER.error") as mock_logger:
            monitoring_service.start_monitoring()
            mock_logger.assert_called_once_with("Failed to initialize TgtgService due to missing credentials.")

    def test_get_tgtg_service_instance_success(self, monitoring_service):
        with patch("app.services.monitoring_tgtg_service.TgtgService") as mock_tgtg_service:
            mock_instance = mock_tgtg_service.return_value
            mock_instance.login.return_value = None  # Mock login behavior
            result = monitoring_service._get_tgtg_service_instance()
    ...
    def test_get_tgtg_service_instance_login_failure(self, monitoring_service):
        with patch("app.services.monitoring_tgtg_service.TgtgService") as mock_tgtg_service, patch("app.common.logger.LOGGER.error") as mock_logger:
            mock_instance = mock_tgtg_service.return_value
    ...
    def test_monitor_favorites_success(self, monitoring_service):
        mock_tgtg_service = MagicMock()
        mock_tgtg_service.get_favorites_items.return_value = []
        ...
        with patch("app.common.utils.Utils.send_telegram_message") as mock_send_message:
            monitoring_service._monitor_favorites()
            mock_tgtg_service.get_favorites_items.assert_called_once()
            mock_send_message.assert_not_called()

    def test_monitor_favorites_parsing_error(self, monitoring_service):
        mock_tgtg_service = MagicMock()
        ...
        with patch("app.common.utils.Utils.send_telegram_message") as mock_send_message, \
            ...
            mock_send_message.assert_called_once_with(
                "TooGoodToNotify: System Error - TgtgAPIParsingError encountered: TgtgAPIParsingError: Parsing error"
            )
    ...
    def test_monitor_favorites_connection_error(self, monitoring_service):
        mock_tgtg_service = MagicMock()
